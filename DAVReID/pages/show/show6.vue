		...
		<div class="content">
			...
			<div class="image-container">
				<img v-for="(image, index) in images" :key="index" :src="image.src"
					...
					:class="{ 'image': true, 'red-border': index === images.length - 1 || index === images.length - 2, 'green-border': index > 0 && index !== images.length - 1 && index !== images.length - 2}" />
			</div>
		</div>
	</div>
</template>
	...
	export default {
		data() {
			return {
				...
				images: [{
						src: '/static/Ve_06_q.png', // 更改为目标查询图像的路径
						displayed: true // 直接显示，无需等待
					},
					{
						src: '/static/vehicle/v-6-1.png',
						displayed: false
					},
					{
						src: '/static/vehicle/v-6-2.png',
						displayed: false
					},
					{
						src: '/static/vehicle/v-6-3.png',
						displayed: false
					},
					{
						src: '/static/vehicle/v-6-4.png',
						displayed: false
					},
					{
						src: '/static/vehicle/v-6-5.png',
						displayed: false
					}
				]
			};
